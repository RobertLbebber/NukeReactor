...
export const uniquenessCondition = (model, item) => {
  ...
  for (let field in model.props) {
    ...
    if (fieldValue.unique && !_.isNil(item[field])) {
      ...
      ExpressionAttributeValues = { ...ExpressionAttributeValues, [`:${field}`]: item[field] };
      ExpressionAttributeNames = { ...ExpressionAttributeNames, [`#${field}`]: field };
    }
  }
  return {
...
export const dynamoDBFilterMarshalling = (Model, filterExpression) => {
  ...
  for (let key in filterExpression) {
    ...
    let type = Model.props[key].type.dbName;
    marshall.ExpressionAttributeValues[`:${key}`] = { [type]: filterExpression[key] };
  }
};
